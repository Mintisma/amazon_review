...
def get_review(asin, page):
    ...
    def get_star(review):
        ...
        star = float(star.replace(',', '.').split()[0])
        return star
    ...
    def get_helpful(review):
        helpful = review.xpath(
            ...
            '')
        try:
            helpful = int(helpful.replace('Eine', '1').split()[0].replace(',', ''))
        except Exception as e:
            helpful = 0
        return helpful

    def get_reviewDate(review):
        dic = {'Januar': 'January', 'Februar': 'February', 'März': 'March', 'Mai': 'May',
        ...
        review_date = review_date.replace('am ', '')
        review_date = replace_all(review_date, dic)
    ...
    time.sleep(1)
    url = 'https://www.amazon.de/product-reviews/'
